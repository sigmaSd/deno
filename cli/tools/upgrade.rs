...
pub static DENORT_ARCHIVE_NAME: Lazy<String> =
  Lazy::new(|| format!("denort-{}.zip", env!("TARGET")));
...
fn get_denort_binary_cache_path(
  dl_dir: &Path,
  version: &str,
  release_channel: ReleaseChannel,
) -> PathBuf {
  let binary_path_suffix = match release_channel {
    ReleaseChannel::Canary => {
      format!("canary/{}/{}", version, *DENORT_ARCHIVE_NAME)
    }
    _ => {
      format!("release/v{}/{}", version, *DENORT_ARCHIVE_NAME)
    }
  };
  dl_dir.join(binary_path_suffix)
}
...
pub async fn upgrade(
  ...
  fs::set_permissions(&new_exe_path, permissions.clone()).with_context(|| {
  ...
  if upgrade_flags.full {
    let denort_cache_path = get_denort_binary_cache_path(
      &dl_dir,
      &selected_version_to_upgrade.version_or_hash,
      requested_version.release_channel(),
    );
    let denort_archive_data =
      if let Some(data) = try_read_cached_binary(sys, &denort_cache_path) {
        log::info!(
          "{}",
          colors::gray(format!(
            "Using cached denort binary from {}",
            denort_cache_path.display()
          ))
        );
        data
      } else {
        let denort_download_url = get_denort_download_url(
          &selected_version_to_upgrade.version_or_hash,
          requested_version.release_channel(),
        )?;
        log::info!(
          "{}",
          colors::gray(format!("Downloading {}", &denort_download_url))
        );
        let Some(data) = download_package(&client, denort_download_url).await?
        else {
          log::error!("denort download could not be found, aborting");
          deno_runtime::exit(1)
        };
        store_cached_binary(sys, &denort_cache_path, &data);
        data
      };

    let denort_temp_dir = tempfile::TempDir::new()
      .context("failed to create temporary directory for denort")?;
    let new_denort_path =
      archive::unpack_into_dir(archive::UnpackArgs {
        exe_name: "denort",
        archive_name: &DENORT_ARCHIVE_NAME,
        archive_data: &denort_archive_data,
        is_windows: cfg!(windows),
        dest_path: denort_temp_dir.path(),
      })
      .context("failed to extract denort archive")?;
    fs::set_permissions(&new_denort_path, permissions).with_context(|| {
      format!(
        "failed to set permissions on '{}'",
        new_denort_path.display()
      )
    })?;

    let denort_output_path = output_exe_path
      .parent()
      .unwrap_or_else(|| Path::new("."))
      .join(if cfg!(windows) { "denort.exe" } else { "denort" });

    let denort_output_result =
      if denort_output_path.exists() && *output_exe_path == current_exe_path {
        replace_exe(&new_denort_path, &denort_output_path)
      } else {
        fs::rename(&new_denort_path, &denort_output_path)
          .or_else(|_| {
            fs::copy(&new_denort_path, &denort_output_path).map(|_| ())
          })
      };
    check_windows_access_denied_error(
      denort_output_result,
      &denort_output_path,
    )?;
    drop(denort_temp_dir);
  }
...
fn get_denort_download_url(
  ...
  let download_url = match release_channel {
    ReleaseChannel::Stable => {
      ...
      format!(
        "{}/download/v{}/{}",
        release_url, version, *DENORT_ARCHIVE_NAME
      )
    }
    ReleaseChannel::Rc => {
      format!("{}/v{}/{}", DL_RELEASE_URL, version, *DENORT_ARCHIVE_NAME)
    }
    ReleaseChannel::Canary => {
      format!("{}/{}/{}", CANARY_URL, version, *DENORT_ARCHIVE_NAME)
    }
    ReleaseChannel::Lts => {
      format!("{}/v{}/{}", DL_RELEASE_URL, version, *DENORT_ARCHIVE_NAME)
    }
  ...
  Url::parse(&download_url).with_context(|| {
    format!(
      "Failed to parse URL to download new denort release: {}",
      download_url
    )
  })
}
